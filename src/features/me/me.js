...
const SENSORY_TOOLS = [
  'Headphones', 'Sunglasses', 'Weighted blanket',
  'Fidget', 'Hoodie', 'Quiet room', 'Familiar music', 'Movement',
];
...
// Colour look-up for link card icon backgrounds and icon fills
const LINK_BG = {
...
export function renderMe() {
    ...
    <div class="screen">
      ...
      <div style="display:flex;gap:6px;margin-bottom:12px;flex-wrap:wrap" role="tablist">
        ${SUPPORT_TABS.map(t => `
          <button
            role="tab"
            aria-selected="${state.supportTab === t.k}"
      ...
      ${links.map(lk => `
        <a href="${lk.url}" target="_blank" rel="noopener noreferrer" class="link-card">
          <div class="link-icon" style="background:${LINK_BG[lk.color]}">
            <i class="ti ${lk.icon}" aria-hidden="true" style="color:${LINK_ICON_COLOR[lk.color]}"></i>
          </div>
          <div style="flex:1">
      ...
      <div class="card" style="padding:0.5rem 1.25rem">
        ${SCRIPTS.map(s => `
          <div class="task-row">
            ...
            <button
              onclick="copyScript('${s.t.replace(/'/g, "\\'")}', this)"
      ...
      <!-- Preferences -->
      <div class="section-label">
        <i class="ti ti-settings" style="color:var(--lavender);font-size:14px" aria-hidden="true"></i>
        Preferences
      </div>

      <div class="card">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          <label for="pref-reminders" style="font-size:14px;font-weight:700">Reminders</label>
          <select id="pref-reminders"
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          <label for="pref-lowstim" style="font-size:14px;font-weight:700">Low-stimulation mode</label>
          <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
            <input type="checkbox" id="pref-lowstim">
            <span id="lowstim-label" style="font-size:13px;color:var(--text-muted)">Off</span>
          </label>
        </div>

        <div style="display:flex;justify-content:space-between;align-items:center">
          <label for="pref-tldr" style="font-size:14px;font-weight:700">TL;DR output style</label>
          <select id="pref-tldr"
            style="font-size:13px;padding:6px 10px;
                   border:2px solid var(--border);border-radius:var(--r-md);
                   background:var(--bg-card);color:var(--text-primary);
                   font-family:var(--font);font-weight:700;cursor:pointer">
            <option>Three bullets</option>
      ...
      <div class="notice green" style="margin-top:0.25rem">
  ...
  document.getElementById('pref-lowstim')?.addEventListener('change', function () {
