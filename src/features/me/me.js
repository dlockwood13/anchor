...
export function renderMe() {
  ...
  document.getElementById('content').innerHTML = `
      ...
      <div style="background: #fff; border: 1.5px solid #e2e8f0; border-radius: 12px; overflow: hidden; margin-bottom: 32px;">
        ${ESSENTIALS.map((e, idx) => `
          <div style="padding: 16px; display: flex; align-items: center; gap: 16px; border-bottom: ${idx < ESSENTIALS.length - 1 ? '1.5px solid #e2e8f0' : 'none'}; cursor: pointer;"
               onclick="const check = this.querySelector('.task-check'); check.classList.toggle('done'); check.innerHTML = check.classList.contains('done') ? '<i class=\\'ti ti-check\\' style=\\'font-size:16px\\'></i>' : '';">
    ...
    <style>
      /* Ensure the custom checklist toggles correctly */
      .task-check.done {
        background: #10b981;
...
window.copyScript = (text, btn) => {
  ...
  setTimeout(() => { btn.innerHTML = '<i class="ti ti-copy" style="font-size: 20px;"></i>'; }, 1500);
};
